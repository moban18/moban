...
class VipController extends CommonController {
    ...
    public function content($id){
        ...
        if($id ){
            ...
            $goodsInfo1=M('GoodsInfo');
            $goodsInfo=$goodsInfo1->where(array('goods_id'=>$info['id']))->find();
            ...
            //商品内空模板里正文头部下面导航内容
            //正文中上一条数据，下一条数据开始
            $preSql='select * from moban_goods where id=(select id from moban_goods where id< '.$id.' limit 1)';
            $pre=$goods->query($preSql);
            $nexSql='select * from moban_goods where id=(select id from moban_goods where id> '.$id.' limit 1)';
            $nex=$goods->query($nexSql);
            //正文中上一条数据，下一条数据结束
            //下载权限，会员种类
            if($info['goods_vips']){
           ...
           //左则推荐模板区开始
                //相关模板
            if($searchs){
                $goods_ids=M('search')->field('goods_id')->where(array('attrter_id'=>$searchs[0]['attrter_id']))->limit(20)->select();
                $xgGoodsIds=$this->xgGoodsId($goods_ids);//处理相关模板数组；
                $xgGoods=$goods->field('id,goods_name,goods_time')->where(array('id'=>array('IN',$xgGoodsIds)))->select();
            }else{
                $xgGoods=$goods->field('id,goods_name,goods_time')->order('goods_comment Desc ')->limit(20)->select();
            }
                //推荐模板
            $tjGoods=$goods->field('id,goods_name,goods_time')->where(array('goods_tuijian'=>1))->order('goods_time Desc')->limit(20)->select();
                //标签云
            $tabs=M('attrter')->where(array('attr_id'=>array('IN','5,6,7,8')))->select();
                //最新模板
            $newGoods=$goods->field('id,goods_name,goods_time')->order('goods_time Desc')->limit(20)->select();
           //左则推荐模板区结束
            if($info){
                $this->getCates();
                $this->assign(array(
    ...
    //左则相关模板处理
    public function xgGoodsId($goods_ids){
        $newGoodsIds=array();
        if(isset($goods_ids)){
            foreach ($goods_ids as $k=>$v) {
                $newGoodsIds[]=$v['goods_id'];
            }
            return $newGoodsIds;
        }else{
            return false;
