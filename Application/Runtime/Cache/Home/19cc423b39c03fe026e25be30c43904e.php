...
<div id="main" class="clearfix">
                ...
                <div class="tg_classify_wrap clearfix ">
                    ...
                    <dl class="tg_classify_wrap_right">
                        ...
                        <dd class="<?php if(!I('filter') or $a[$ok]==0){echo 'on1';}?>">
                            ...
                            <a href="<?php
 if($filter=='0-0-0-0-0-0'){ echo U('Video/index'); }elseif(!I('filter')){ echo U('Video/index'); }else{ echo U('Video/index',['filter'=>$filter]); } ?>">
                                </span>不限</span>
                            </a>
                        </dd>
 ...
 if(!I('filter') or $feilei==0){ echo 'on1'; } if(in_array($feilei,$parent_ids)){ echo 'on1'; } ?>">
                            ...
                            <a href="<?php
 if($filter){ if(in_array($feilei,$parent_ids)){ $n[$ok]=$feilei; }else{ foreach($attrterss as $x){ if($feilei==$x['attrter_id']){ $n[$ok]=$x['attr_id']; } } } $filter=implode('-',$n); echo U('Video/index',['filter'=>$filter]); } ?>">
 ...
 if($L[$ok]<=0){ echo 'javascript:void(0);'; }else{ echo U('Video/index',['filter'=>$pre]); } ?>">
 ...
 if($filter){ $p=explode('-',$filter); } ?>
                    ...
                    <a class="pnext-btn page-btn" href="<?php echo U('Video/index',['filter'=>$nex])?>">
 ...
 if($L[$ok]>=$pages){ echo 'style="border-left: 5px solid #CCCCCC;"'; } ?>></span>
            ...
            <div id="list" class="list">
                <div class="list_box clearfix">
                    ...
                    <div class="goods_box">
                        <a href="/moban/index.php/Home/Video/content/id/<?php echo ($v['id']); ?>"><img src="/moban/<?php echo ($v['goods_photo']); ?>" alt=""/></a>
                        <p class="goods_title">
            ...
            <div class="page_list clearfix">
                ...
                <?php if(I('filter')){ $L=explode('-',I('filter')); $L[$ok]= $L[$ok]-1; $pre=implode('-',$L); }?>
                <a href="<?php
 if($L[$ok]<=0){ echo 'javascript:void(0);'; }else{ echo U('Video/index',['filter'=>$pre]); } ?>" class="pre">上一页</a>
 ...
 if(I('filter')){ $z=explode('-',I('filter')); $curr=$z[$ok]; if($curr=='' || $curr==0){ $curr=1; } }else{ $z=array_fill(0,$num+1+1,0); $curr=1; } ?>
                ...
                <a <?php if($curr==$i):?>
                    ...
                    <?php else:?>
                        class="num"
                        href= <?php echo U('Video/index',['filter'=>$filter])?>
                    <?php endif;?>
                ><?php echo ($i); ?></a>
                <?php endfor;?>
                <!--<a href="###" class="end">...107</a>-->
                <?php if(I('filter')){ $L=explode('-',I('filter')); $L[$ok]= $L[$ok]+1; if($L[$ok]>=$pages){ $L[$ok]=$pages; } $nex=implode('-',$L); }else{ $L=array_fill(0,$num+1+1,0); $L[$ok]= 1; $L[$ok]=$L[$ok]+1; $nex=implode('-',$L); } ?>
                <a href="<?php echo U('Video/index',['filter'=>$nex])?>" class="next">下一页</a>
