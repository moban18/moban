...
<div id="main" class="clearfix">
 ...
 if($L[$ok]<=0){ echo 'javascript:void(0);'; }else{ echo U('Vip/index',['filter'=>$pre]); } ?>">
 ...
 if($L[$ok]>=$pages){ echo 'style="border-left: 5px solid #CCCCCC;"'; } ?>></span>
            ...
            <div class="page_list clearfix">
                ...
                <?php if(I('filter')){ $L=explode('-',I('filter')); $L[$ok]= $L[$ok]-1; $pre=implode('-',$L); }?>
                <a href="<?php
 if($L[$ok]<=0){ echo 'javascript:void(0);'; }else{ echo U('Vip/index',['filter'=>$pre]); } ?>" class="pre">上一页</a>
 ...
 if(I('filter')){ $z=explode('-',I('filter')); $curr=$z[$ok]; if($curr=='' || $curr==0){ $curr=1; } }else{ $z=array_fill(0,$num+1+1,0); $curr=1; } ?>
                ...
                <a <?php if($curr==$i):?>
                    ...
                    <?php else:?>
                        class="num"
                        href= <?php echo U('Vip/index',['filter'=>$filter])?>
                    <?php endif;?>
                ><?php echo ($i); ?></a>
                <?php endfor;?>
